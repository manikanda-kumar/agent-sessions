...
final class AmpSessionIndexer: ObservableObject, SessionIndexerProtocol, @unchecked Sendable {
    ...
    @AppStorage(PreferencesKey.Paths.ampSessionsRootOverride) var sessionsRootOverride: String = ""
    ...
    private var discovery: AmpSessionDiscovery
    ...
    init() {
        let initialOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.ampSessionsRootOverride) ?? ""
        self.discovery = AmpSessionDiscovery(customRoot: initialOverride.isEmpty ? nil : initialOverride)
    ...
    func refresh(mode: IndexRefreshMode = .incremental,
        ...
        if !AgentEnablement.isEnabled(.amp) { return }

        let currentOverride = UserDefaults.standard.string(forKey: PreferencesKey.Paths.ampSessionsRootOverride) ?? ""
        if currentOverride != lastOverride {
            discovery = AmpSessionDiscovery(customRoot: currentOverride.isEmpty ? nil : currentOverride)
        ...
        Task.detached(priority: prio) { [weak self, token, executionProfile] in
            ...
            let config = SessionIndexingEngine.ScanConfig(
                ...
                parseLightweight: { AmpSessionParser.parseFile(at: $0) },
    ...
    func reloadSession(id: String, force: Bool = false, reason: ReloadReason = .selection) {
        ...
        ioQueue.async {
            ...
            let parsed = AmpSessionParser.parseFileFull(at: url) ?? existing
            self.reloadLock.lock()
